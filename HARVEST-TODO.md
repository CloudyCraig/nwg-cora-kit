# Harvest TODO — ALL DONE 2026-09-10 (kept for the record)

Everything below was captured while the Splunk box was powered on: see
`itsi-export/`, `splunk-box/` and `cluster-snapshot/`.

- [x] **ITSI full backup** — Settings → Backup/Restore (or `kvstore_op=backup`):
...
- [x] **craigs_digital_experience glass table** — export via
      `itoa_interface/glass_table/craigs_digital_experience` (no local copy).
- [x] **Box nginx config** `/etc/nginx/conf.d/spa.conf` (SPA/API/HEC proxy,
...
- [x] **HEC token config + indexes** (nwpay_audit / nwpay_infra / thousandeyes / itsi_* )
      and props/transforms from cloud-init.
- [x] Chaos-controller + payments-app manifests (Marc has the original IaC; the kit
