> ## ⚠️ DO NOT REVERT: ledger-service must stay >= `0.1.7`
> The db-slow demo (real Postgres `SELECT pg_sleep()` → HikariCP pool
> exhaustion → 5xx cascade → ITSI tree deflection) **only works on
> `ledger-service` image `0.1.7` or newer**. Builds `<= 0.1.1` do a Java-side
> `Thread.sleep` only: **no pg_sleep span, no pool drama, no ITSI impact** —
> the demo looks broken.
>
> This already regressed once (weekend of 2026-06-20/21) when a `helm upgrade`
> re-pinned the tag back to the old build. The tag is now pinned in three
> places — **keep all three in sync, do not lower them**:
> - `helm/natwest-payments/values.yaml` → `services.ledger-service.image.tag: "0.1.7"`
> - `scripts/03-deploy.sh` → `LEDGER_JAVA_TAG="${LEDGER_JAVA_TAG:-0.1.7}"`
> - `scripts/05-deploy-frontend.sh` → `LEDGER_JAVA_TAG="${LEDGER_JAVA_TAG:-0.1.7}"`
>
> Only raise the tag (after rebuilding a newer pg_sleep image); **never** set it
> below `0.1.7` or let it fall back to `IMAGE_TAG`. After any deploy, confirm:
> `kubectl -n natwest get deploy ledger-service -o jsonpath='{..image}'` shows `:0.1.7`+.
> Full root-cause writeup: see the **2026-06-22** section at the bottom of this file.
...
- **Node-colour fix (2026-06-21).** `nwpay_l4_postgres` now carries four
  chaos-gated KPIs only (HikariCP pool in use / pending / timeouts + postgres
  backends online), with `severity_aggregation: max`. During db-slow the JDBC
  pool KPIs go critical and the Service Analyzer node turns red; during
  postgres-outage the backends KPI trips at 0. Re-run
  `scripts/07-itsi-bootstrap.sh` after pulling this change.
- **Node-colour caveat (superseded).** The previous eight-KPI equal-weight
  score kept the node green when only one KPI deflected. That layout is
  replaced by the four-KPI max-aggregation model above.
...
| 2 | **Splunk Web → RUM → Sessions** | Filter `enduser.id = cust-uk-003` (or `enduser.name = Margaret`), open the latest session. | Page-action span for `/api/process` shows duration ~700 ms. | RUM session captures the real-world UX. |
...
| 7 | **ITSI → Service Analyzer → `nwpay_l4_postgres`** | Click. | Service Analyzer **node turns red**; KPI tiles show JDBC pool in use at 16/16, pending requests, and connection timeouts climbing. | Postgres + app-side pool monitoring — chaos-gated KPIs tied to db-slow. |
...
## 2026-06-22 — db-slow regressed over the weekend (ledger rolled back to 0.1.1)

**Symptom:** demoed db-slow, no `pg_sleep` injected, no Hikari pool drama, ITSI
tree unaffected.

**Root cause:** `ledger-service` was running image `0.1.1` again — the
**pre-`pg_sleep`** build (Java-side `Thread.sleep`, no pooled-connection hold,
no DB span). A weekend `helm upgrade` (`scripts/03-deploy.sh`) re-pinned the
tag via `--set services.ledger-service.image.tag=${LEDGER_JAVA_TAG}`, and
`LEDGER_JAVA_TAG` defaulted to `IMAGE_TAG` (0.1.0/0.1.1). My Friday
`kubectl set image …:0.1.7` was a raw pod-spec edit, not a Helm value, so the
Helm run discarded it. chaos-controller `0.1.9` (1500ms) was untouched.

**Fix (running):** `kubectl set image deploy/ledger-service service=…/natwest-payments-ledger-service-java:0.1.7 -n natwest`.

**Fix (durable, so it can't silently revert again):**
- `helm/natwest-payments/values.yaml` → `services.ledger-service.image.tag: "0.1.7"` (was 0.1.0).
- `scripts/03-deploy.sh` and `scripts/05-deploy-frontend.sh` → `LEDGER_JAVA_TAG` now defaults to `0.1.7` (was `${IMAGE_TAG}`). Bump only after rebuilding pg_sleep.

**Verified live under db-slow (2026-06-22):**
- Postgres `pg_stat_statements`: `SELECT pg_sleep($1)` — 460 calls, mean 1501ms.
- ledger logs: `HikariPool-1 - Connection is not available, request timed out after 3000ms (total=16, active=16, idle=0, waiting=68)`.
- ITSI metric `db.client.connections.usage{state=used}` ramped 0–1 → **16** (KPI critical = 14); `db.client.connections.timeouts` ~530.
- Scenario then **cleared** (DB_LATENCY_MS 1500→0) — armed/baseline for the next demo.

**Access note:** my workstation IP was added to the EKS `publicAccessCidrs`
allowlist (`213.18.180.180/32`) to reach the API; can be removed when no longer
needed. Use `AWS_PROFILE=marc` + `aws eks update-kubeconfig --name natwest-payments-demo --region eu-west-2`.

**Inject/clear by hand (header `X-Chaos-Token`, token in secret `chaos-controller-token`):**
```
kubectl exec -n natwest deploy/chaos-controller -- python -c \
 "import urllib.request as u;u.urlopen(u.Request('http://localhost:8080/chaos/api/db-slow/inject',data=b'{}',method='POST',headers={'X-Chaos-Token':'<TOKEN>'}))"
# …/db-slow/clear to reset.
```
