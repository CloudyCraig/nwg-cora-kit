...
# traffic-generator/deployment.yaml steady-state load shape. The demo now runs
# a flat 12 rps baseline with NO time-of-day schedule: the old curve peaked at
# 40-50 rps, which saturates the payment fan-out on the current cluster
# (multi-second tail + 6 s read timeouts) and masks injected chaos. Keep these
# in sync with traffic-generator/deployment.yaml's RPS / RPS_SCHEDULE env so
# clear() restores the green baseline rather than re-arming the saturating peak.
BASELINE_TRAFFIC_RPS = "12"
...
# Real-path Spain degradation. Arms the gated SLOW_LOCATION_* knobs on
# sanctions-aml-service so REAL customer payments with customer_country=ES
# (the Sofia / Elena Madrid personas in the SPA) get ~2 s extra latency and a
# portion fail with a descriptive RegionalDependencyError 5xx - giving a clean
# RUM -> APM root-cause trace. Latency (2 s) sits well under the 6 s default
# downstream timeout so the caller sees a clean long/errored span, not a
# read-timeout cascade. The scenario ALSO arms the synthetic traffic-generator
# Madrid profile so the existing "p95 by customer.location" / ITSI Madrid
# panels light up. Default OFF; clear() restores both to baseline.
SPAIN_DEGRADE_SERVICE = "sanctions-aml-service"
SPAIN_DEGRADE_COUNTRY = "ES"
DEFAULT_SPAIN_DEGRADE_LATENCY_MS = 2000.0
DEFAULT_SPAIN_DEGRADE_JITTER_MS = 400.0
DEFAULT_SPAIN_DEGRADE_ERROR_RATE = 0.9
...
def _inject_madrid_payment_degradation(params: dict[str, Any] | None) -> dict[str, Any]:
    """Degrade REAL Spain (ES) SPA payments at sanctions-aml-service.

    Arms the gated SLOW_LOCATION_* knobs so payments carrying
    customer_country=ES (the Sofia / Elena Madrid personas) get ~latency_ms of
    extra latency and a `error_rate` fraction return a descriptive
    RegionalDependencyError 5xx (visible as the root cause in the APM trace).
    Also arms the synthetic traffic-generator Madrid profile so the existing
    p95-by-location dashboards / ITSI Madrid episode light up.

    Optional params:
      latency_ms  - extra latency for ES payments (default 2000, [50, 5000])
      error_rate  - fraction of ES payments that 5xx (default 0.5, [0, 1])
    """
    p = params or {}
    try:
        latency_ms = float(p.get("latency_ms", DEFAULT_SPAIN_DEGRADE_LATENCY_MS))
        error_rate = float(p.get("error_rate", DEFAULT_SPAIN_DEGRADE_ERROR_RATE))
    except (TypeError, ValueError) as exc:
        raise ValueError(
            f"madrid-payment-degradation requires numeric latency_ms/error_rate, got {p!r}"
        ) from exc
    # Bound latency under the 6 s downstream timeout so we get clean errored
    # spans, not read-timeout cascades.
    if not (50.0 <= latency_ms <= 5_000.0):
        raise ValueError("madrid-payment-degradation latency_ms must be in [50, 5000]")
    if not (0.0 <= error_rate <= 1.0):
        raise ValueError("madrid-payment-degradation error_rate must be in [0.0, 1.0]")
    results: dict[str, Any] = {}
    results["payment_path"] = k8s.set_env(
        SPAIN_DEGRADE_SERVICE,
        {
            "SLOW_LOCATION_COUNTRY": SPAIN_DEGRADE_COUNTRY,
            "SLOW_LOCATION_LATENCY_MS": f"{latency_ms:.0f}",
            "SLOW_LOCATION_JITTER_MS": f"{DEFAULT_SPAIN_DEGRADE_JITTER_MS:.0f}",
            "SLOW_LOCATION_ERROR_RATE": f"{error_rate:.2f}",
        },
    )
    # Light up the existing synthetic Madrid dashboards / ITSI episode too.
    results["traffic_generator"] = k8s.set_env(
        "traffic-generator", {"LOCATION_LATENCY_PROFILES": DEFAULT_MADRID_DEGRADED_PROFILE}
    )
    return results


def _clear_madrid_payment_degradation() -> dict[str, Any]:
    results: dict[str, Any] = {}
    results["payment_path"] = k8s.set_env(
        SPAIN_DEGRADE_SERVICE,
        {
            "SLOW_LOCATION_COUNTRY": "",
            "SLOW_LOCATION_LATENCY_MS": "0",
            "SLOW_LOCATION_JITTER_MS": "0",
            "SLOW_LOCATION_ERROR_RATE": "0",
        },
    )
    results["traffic_generator"] = k8s.set_env(
        "traffic-generator",
...
def _status_madrid_payment_degradation() -> dict[str, Any]:
    return _status_from_env(
        SPAIN_DEGRADE_SERVICE,
        "SLOW_LOCATION_COUNTRY",
        lambda v: v.strip() != "",
...
def _repair_apm_topology() -> dict[str, Any]:
    ...
    for deploy in (
        ...
        if k8s.deployment_exists(deploy):
            results[f"{deploy}_peer_mapping"] = k8s.set_env(
                deploy,
                {
                    "OTEL_INSTRUMENTATION_COMMON_PEER_SERVICE_MAPPING": (
                        APM_PEER_SERVICE_MAPPING
    ...
    if k8s.deployment_exists("settlement-service"):
        results["settlement_kafka_env"] = k8s.set_env(
            "settlement-service",
            {
    ...
    # Restore infra deployments after postgres-outage / redis-outage / kafka-outage.
    for infra_deploy in ("postgres", "redis", "kafka"):
        if not k8s.deployment_exists(infra_deploy):
            continue
        current = k8s.current_replicas(infra_deploy)
        if current is not None and current < 1:
            results[f"scale_{infra_deploy}"] = k8s.scale(infra_deploy, 1)
...
CATALOG: tuple[Scenario, ...] = (
    ...
    Scenario(
        ...
        narrative=(
            ...
            "the whole arc lands in one ITSI Episode. Canonical SPA context: "
            "Margaret (Gold), FPS £25 to Henry — Pocket money. Use for the "
            ...
            "HikariCP pool metrics all light up. See docs/customer/story-"
            "rum-apm-postgres.md for the click-by-click talk track."
        ),
        target_service="ledger-service + postgres",
        watch=(
            "ACT 1 (db-slow, ~4 min): Margaret (Gold) FPS submit wait in "
            "RUM / replay; APM ledger-service red; Database Query Performance "
            "pg_sleep top-N; ITSI nwpay_l4_postgres node red (JDBC pool KPIs). "
    ...
    Scenario(
        ...
        category="latency",
        narrative=(
            "Inject 1500 ms latency into every ledger-service Postgres query. "
        ...
        watch=(
            "Margaret (Gold) FPS submit in RUM / replay. "
            "APM service map: ledger-service red, postgres edge slow. "
            "APM Database Query Performance: SELECT pg_sleep top-N. "
            "ITSI nwpay_l4_postgres: JDBC pool in use / pending / timeouts KPIs; "
            "Service Analyzer node turns red. "
    ...
    Scenario(
        id="madrid-payment-degradation",
        name="Madrid payment degradation (real SPA payments)",
        category="latency",
        narrative=(
            "Degrade the EU-South AML screening dependency for ES-region payments "
            "on sanctions-aml-service. REAL SPA payments from the Madrid personas "
            "(Sofia / Elena, customer_country=ES) get ~2 s extra latency and most "
            "(~90%) fail with a descriptive RegionalDependencyError 5xx. Also arms the "
            "synthetic traffic-generator Madrid profile so the existing p95-by-"
            "location and ITSI Madrid panels light up. Full RUM -> APM root-cause "
            "story: SPA slow -> RUM session (Spain) -> APM trace -> sanctions-aml "
            "5xx + recorded exception (the 'why')."
        ),
        target_service="sanctions-aml-service + traffic-generator",
        watch=(
            "SPA: a Madrid persona (Sofia) payment is slow / degraded. "
            "RUM: filter customer.country=ES, open the session -> slow payment.completed. "
            "APM: sanctions-aml-service red; trace shows the 500 span with the "
            "RegionalDependencyError exception ('why'); 'Logs for this trace' shows the "
            "correlated regional_dependency_timeout log. Dashboards: p95 by "
            "customer.location (Madrid) + ITSI Madrid episode."
        ),
        recovery_hint=(
            "Clear restores SLOW_LOCATION_*='' on sanctions-aml-service and "
            "LOCATION_LATENCY_PROFILES='' on the traffic generator."
        ),
        severity="high",
        inject=_inject_madrid_payment_degradation,
        clear=_clear_madrid_payment_degradation,
        status=_status_madrid_payment_degradation,
    ),
    Scenario(
